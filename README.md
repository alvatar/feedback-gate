...
- `cloudflare/worker.mjs` + `cloudflare/feedback-core.mjs`: public ingestion endpoint, host allowlist, CORS, honeypot, rate limit, forwarder
...
7. Deploy the Worker.
8. Point DNS at Cloudflare and bind a route such as `feedback.example.com`.
9. Set `ALLOWED_HOSTS` to that public host so the Worker rejects `workers.dev` and any other hostnames.
...
2. Set `endpoint` to the deployed public custom-domain URL.
